...
### 第二步：扫描已安装技能
列出 `~/.Codex/skills/` 目录下的所有技能。

### 第三步：分析历史记录
解析以下文件：
- `~/.Codex/history.jsonl`（全局历史）
- `~/.Codex/projects/*/session.jsonl`（项目会话）
...
- **未使用技能**：在结果中显示为 0 次
- **历史文件位置**：`~/.Codex/history.jsonl` 和 `~/.Codex/projects/*/*.jsonl`
...
2. 直接调用 `~/.Codex/skills/skill-usage/stats.py` 脚本，传入时间段参数
...
调用示例：
```bash
python3 ~/.Codex/skills/skill-usage/stats.py past_30_days
```
